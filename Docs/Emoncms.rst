...
The graph application used locally by Maclogger is a derivitive
of that effort.
...
Configuring an Maclogger to upload to Emoncms is easy. 
...
to a few dollars per month for a typical Maclogger user.

Configure Maclogger
------------------

After establishing an account, run the Maclogger 
...
**post interval**
    ...
    time (larger interval). Maclogger accepts any value 
...
**bulk send**
    ...
    issues. By specifying a larger bulk send, Maclogger will 
...
**upload history from**
    ...
    Maclogger will bulk upload the historical data to Emoncms. 
...
**Emoncms userid**
    By specifying this optional value, you instruct Maclogger to use 
...
Now click |save| and Maclogger should begin sending data to 
your Emoncms account. You can see the status of the 
Emoncms service in the `Status Display <status.html#web-servers>`__

Maclogger uploads the current voltage or power corresponding 
...
Initially, these fields correspond to each of the Maclogger inputs. 
