...
any of the data stored in the Maclogger. It features relative and absolute time period selection 
and handles all units of measure supported by Maclogger. You can produce line and/or bar charts 
...
On the right Watts are selected and this Maclogger lists 15 sources that are configured 
...
Maclogger records each measurement at 5 second intervals. That's 17,280
measurements per day. The graph size cannot possibly represent that, and it would take
a long time to query that amount of data.  Instead, Maclogger can deliver either
...
Each graph is saved in the Maclogger, so it doesn't matter if you use a different 
...
Graph+ can be loaded directly from the Maclogger's web server using the URL

http://Maclogger.local/graph2.htm [?graph=savedgraph [&embed] ]
