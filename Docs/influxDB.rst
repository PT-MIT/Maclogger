...
Maclogger fully supports the influxDB HTTP API for uploading data 
to influxDB at a specific interval of 5 seconds to one hour. 
Like other similar Maclogger services, continuity of updates is 
...
to version 2, depending on the implementation (cloud or OSS), but Maclogger
...
To configure the influxDB upload service in Maclogger, 
...
**post interval**
    ...
    Maclogger accepts any value from 5 seconds to 3600 seconds (1 hour), 
...
**bulk send**
    ...
    Maclogger will aggregate the posting data for the specified number 
    ...
    Maclogger keeps all of the data in the SD data log, 
    so there is no risk of losing data by using this feature. 
    Should there be any failure to deliver, 
    Maclogger will pick up with the last successful posting when the problem is resolved.
...
**upload history from**
    Specify the starting date that Maclogger will use to upload history 
...
**measurement**
    Name that you assign to the measurements that Maclogger will be posting. 
...
**database**
    ...
    have created to be the repository for the Maclogger data.
...
**username/password** 
    Optional security credentials. If specified, Maclogger will 
...
**Authorization token**
    Authorization token produced by influx. (Data->Tokens->Generate Token-> Read/Write Token for Maclogger bucket)
...
**tag-set** 
    ...
    The first tag-set is a special case for Maclogger, and if specified, 
...
**measurements** 
    ...
    create scripts to generate data using the various Maclogger inputs. 
...
There are three variables defined: 
    ...
    *   **$device** - The name assigned to this Maclogger device 
